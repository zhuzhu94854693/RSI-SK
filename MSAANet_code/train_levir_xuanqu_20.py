...
device = 'cuda'
path = r'/home/user/zly/data2/WHU_clip/sample/t0.20/'
path2 = r'/home/user/zly/data2/WHU_clip/sample/orgion/'
...
def get_dataset_loaders(workers, batch_size=4):
    ...
    val_dataset = ATDataset(
        os.path.join(path2, "val", "A"), os.path.join(path2, "val", "B"), os.path.join(path2, "val", "label"),
        trainval_transform, test_transform, target_transform
    )
...
def train(loader, num_classes, device, net, optimizer, criterion):
    ...
    for images1, images2, masks in tqdm.tqdm(loader):
        ...
        # outputs2 = outputs[:, 1, :, :].unsqueeze(1)
        masks = masks[:, 0, :, :]#.unsqueeze(1)
        # plt.imshow(outputs[0, :, :, :].cpu().detach().numpy())
        # plt.show()

        loss = criterion(outputs, masks.float())
        # loss = criterion(outputs1, masks1.long())
        loss.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', nargs='?', type=float, default=5e-3,
                        help='Learning Rate')
    parser.add_argument('--model', nargs='?', type=str, default='MSAANet')
    ...
    logger = get_log("MSAANet" + 't0.20_train_log.txt')

    scheduler = PolynomialLRDecay(opt, max_decay_steps=100, end_learning_rate=0.0001, power=2.0)

    # Initialize a variable to store the best F1 score
    best_f1 = 0.0
    ...
    for epoch in range(num_epochs):
        ...
        logger.info(('loss={}'.format(val_hist["loss"]),
                     ...
                     'iou={}'.format(val_hist["iou"]),
                     'oa={}'.format(val_hist["oa"])))
        f1score = val_hist["iou"]
        ...
        if f1score > best_f1:
            ...
            checkpoint = os.path.join(
                # r'/home/user/zly/data2/WHU_clip/pth/MSAANet/t0.20/','model{}_{}_{}_best.pth'.format(model_name, f1score,epoch)
                r'/home/user/zly/data2/WHU_clip/pth/MSAANet/t0.20/model_best.pth'
            )
            torch.save(net, checkpoint)
            logger.info(f'Saved new best model with F1 score: {best_f1}')
