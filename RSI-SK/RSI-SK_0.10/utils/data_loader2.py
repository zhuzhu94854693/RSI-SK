...
class SemiChangeDataset(data.Dataset):
    def __init__(self, root, trainsize, train_ratio=1,mosaic_ratio=0.75):
        self.trainsize = trainsize
        # get filenames
        self.image_root_A_t005 =  root + 't0.10/train/A/'
        self.image_root_B_t005 =  root + 't0.10/train/B/'
        self.gt_root_t005 = root + 't0.10/train/label/'

        self.image_root_A_shen005 = root + 'orgion/train/A/'
        self.image_root_B_shen005 = root + 'orgion/train/B/'
        self.gt_root_shen005 = root + 'orgion/train/label/'

        self.images_A_t005 = [self.image_root_A_t005 + f for f in os.listdir(self.image_root_A_t005) if
                              f.endswith('.jpg') or f.endswith('.png')]
        self.images_B_t005 = [self.image_root_B_t005 + f for f in os.listdir(self.image_root_B_t005) if
                              f.endswith('.jpg') or f.endswith('.png')]
        self.gts_t005 = [self.gt_root_t005 + f for f in os.listdir(self.gt_root_t005) if
                         f.endswith('.jpg') or f.endswith('.png')]

        self.images_A_shen005 = [self.image_root_A_shen005 + f for f in os.listdir(self.image_root_A_shen005) if
                                 f.endswith('.jpg') or f.endswith('.png')]
        self.images_B_shen005 = [self.image_root_B_shen005 + f for f in os.listdir(self.image_root_B_shen005) if
                                 f.endswith('.jpg') or f.endswith('.png')]
        self.gts_shen005 = [self.gt_root_shen005 + f for f in os.listdir(self.gt_root_shen005) if
                            f.endswith('.jpg') or f.endswith('.png')]

        self.images_A = self.images_A_t005 + self.images_A_shen005
        self.images_B = self.images_B_t005 + self.images_B_shen005
        self.gts = self.gts_t005 + self.gts_shen005
        ...
        self.gt_list = []
        for image_path in self.images_A_t005:
            if image_path in self.images_A_t005:
                self.gt_list.append(self.images_A.index(image_path))


        print('Training SemiNet with ',train_ratio*100, '% label available!')
        print('Total Sample ', len(self.images_A))
        print(int(len(self.images_A_t005)),' patches with label & ',int(len(self.images_A_shen005)),' without label!')
